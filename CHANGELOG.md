...
## v1.10.0 (2026-05-06) - 通知配置增强 / 服务管理 / 文档更新

### 新增功能

#### 1. 通知配置增强
- **飞书通知**：新增飞书群机器人Webhook配置
- **企业微信通知**：新增企业微信群机器人Webhook配置
- **通知配置文档**：新增 `/overview/notify` 页面，详细说明钉钉/飞书/企业微信/邮件配置步骤

#### 2. 服务管理功能
- **Web服务菜单**：左侧菜单新增Web服务子菜单
- **启动/停止/重启**：支持Web服务热管理
- **服务状态**：实时查看服务运行状态和端口
- **自动端口检测**：启动时自动检测端口冲突

#### 3. 巡检指标备份恢复
- **Prometheus指标备份**：巡检指标页面支持备份Prometheus配置
- **ES查询备份**：巡检指标页面支持备份ES查询配置
- **一键恢复**：快速恢复历史配置

#### 4. 系统设置优化
- **Tab跳转修复**：点击AI分析/通知/系统日志直接跳转对应页面
- **子菜单链接**：左侧菜单支持直接跳转各配置页面

### Bug 修复

- **settings.html模板错误**：修复条件渲染导致模板错误
- **datetime导入**：添加 `from datetime import datetime`
- **编辑/复制功能**：修复机房管理编辑复制不生效问题
- **注册页面权限**：开放注册功能，任何用户均可注册

### 代码修改列表

| 文件 | 修改内容 |
|------|---------|
| `src/web/templates/settings.html` | 添加飞书/企业微信配置，Tab条件渲染 |
| `src/web/templates/overview_notify.html` | 新增通知配置文档 |
| `src/web/templates/base.html` | 添加Web服务菜单 |
| `src/web/templates/queries.html` | 添加Prometheus/ES备份恢复按钮 |
| `src/web/api.py` | 添加config备份/恢复API，service管理API |
| `src/web/routes.py` | 添加通知文档路由，settings参数支持 |
| `manage_x.py` | 添加端口自动检测，更新管理脚本 |
| `CHANGELOG.md` | 更新v1.10.0日志 |

---

## v2.0.0 (2026-05-06) - 响应式UI / 备份恢复 / 账号管理
