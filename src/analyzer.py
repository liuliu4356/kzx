...
- 每项注明来源(metric/log)、名称、当前值/片段、阈值或异常原因。
...
3. 如果数据全部正常,也要完整输出四节。
4. 不要输出任何与上述四节无关的内容,不要重复数据原文。"""


def _build_user_payload(prom_results: list[PromResult],
    ...
    if batch_window:
        context["batch_window"] = {
            ...
            "note": (
                f"当前处于【{batch_window.label}】时间窗口。"
                "以下指标在此期间属于已知正常波动，请勿误判为异常："
            ),
...
def analyze(prom_results: list[PromResult],
            es_results: list[ESResult],
            cfg: LLMConfig,
    ...
    user_text = _build_user_payload(prom_results, es_results, batch_window)
